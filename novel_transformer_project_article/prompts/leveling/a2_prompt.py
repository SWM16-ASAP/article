...
def get_prompt(use_rag: bool = False, is_feedback_generated: bool = False) -> ChatPromptTemplate:
    """
    Builds and returns the complete ChatPromptTemplate for the A2 level,
    ...
    level_guidelines = """
    <level_guidelines>
        **Guidelines for A2 Level:**
        - Use simple present, past, and future tenses
        - Use compound sentences with basic connectors (and, but, because, so, when)
        - Use everyday vocabulary with some variety
        - Use basic descriptive language (adjectives and adverbs)
        - Use simple relative clauses (who, which, where)
        - Use basic time expressions and sequencing words
        - Use simple reported speech
        - Use basic modal verbs (can, could, should, must)
        - Use simple passive voice when necessary
        - Use basic phrasal verbs
        - Use simple conditional sentences (if + present, will + infinitive)
    </level_guidelines>"""
    
    critical_instructions = """
    <critical_instructions>
        ⚠️  CRITICAL: ABSOLUTELY NO EXPLANATIONS, LABELS, OR EXTRA TEXT ⚠️
        - DO NOT add any text like "A2 Level:", "Here is the A2 version:", or any explanations
        - DO NOT add any introductions, commentary, or labels and tags
        - ONLY return the A2 level text
        - START DIRECTLY with the A2 text
        - END DIRECTLY with the A2 text

        ⚠️  **CRITICAL: OUTPUT FORMAT EXAMPLE** ⚠️
        <example>
        Expected Output:
        The boy saw a big dog in the park. He was very happy because he loves animals. The dog looked friendly and wagged its tail when the boy approached.

        NOT like this:
        "A2 Level Version: Here is the adapted text..."
        "I have adapted the text to A2 level..."
        "Based on the original, here is the A2 version..."
        </example>
        ...
        - If something cannot be expressed at A2 level, use the simplest possible words
        
    </critical_instructions>"""

    human_prompt_persona = """You are an expert English teacher specializing in adapting texts for A2 level learners (Basic User - Elementary)."""
    ...
    <feedback_instructions>
        ...
        **Feedback:**
        ...
        - ONLY provide the pure A2 level story text - nothing else
        - Act as if this is your first and only attempt at the A2 level text
        ---
    </feedback_instructions>"""
    ...
    <human_prompt_context_and_instructions>
        ...
        Please rewrite this English text for A2 level, considering the following context:
        ...
        **If no feedback:**
        ...
        - Focus on creating high-quality A2 level text
        ...
        *(Text that came immediately before this chunk)*
        ...
        - Make sure your A2 text connects naturally with the previous chunk
        ...
        - Use everyday vocabulary with some variety while preserving the story flow
    </human_prompt_context_and_instructions>"""
    ...
    <rag_system_addition>
        ...
        **Available Tools:**
        You have access to the find_alternative_words tool. Use it for comprehensive A2 level adaptation, not just word replacement.
        - Identify ALL words that don't match A2 level: both too difficult AND too simple words
        - Look for: complex vocabulary, advanced grammar, AND overly basic words that need upgrading
        - Call find_alternative_words(words="word1, word2, word3", target_level="A2", context="actual surrounding text from the passage")
        - This tool provides A2-level alternatives as reference - use them as guidance, not strict requirements
    </rag_system_addition>"""

        rag_human_addition = """<rag_human_addition>

        - **IMPORTANT**: For comprehensive A2 level adaptation:
          1. Analyze the ENTIRE text for A2 level appropriateness
          2. Identify words that are: TOO DIFFICULT (above A2) AND TOO SIMPLE (below A2)
          3. Look for: complex vocabulary, advanced grammar structures, AND overly basic words
          4. Use find_alternative_words with ALL inappropriate words: find_alternative_words(words="complex_word, too_simple_word", target_level="A2", context="full text context")
          5. Choose appropriate A2-level alternatives for each word
        ...
        - **Tool Usage Example:**
        Identify words that don't match A2 level (too difficult OR too simple):
        
        <example>
        Original Text: "The sophisticated design was big and the elaborate system was small."
        
        Step 1: Identify inappropriate words
        → find_alternative_words(words="sophisticated, elaborate, big, small", target_level="A2", context="...")
        ...
        Step 3: Choose appropriate A2 alternatives
        → "The complex design was big and the detailed system was small."
        </example>
        
        Use these alternatives as reference - you don't have to use them exactly. Choose what fits best for A2 level or create your own appropriate alternatives.
    </rag_human_addition>"""
    ...
    final_human_prompt = human_prompt_text + """
        ...
        Return only the A2 level English text. No formatting, no labels, no explanations.
        """

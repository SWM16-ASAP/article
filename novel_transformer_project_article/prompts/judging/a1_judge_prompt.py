...
def get_judge_prompt() -> ChatPromptTemplate:
    """
    A1 레벨 텍스트 품질을 평가하는 Judge AI용 프롬프트를 반환합니다.
    """
    ...
    evaluation_criteria = """
    <evaluation_criteria>
        **A1 Level Evaluation Criteria:**
        
        **1. Vocabulary (25 points)**
        - Uses basic vocabulary (family, food, colors, common objects, common everyday expressions).
        - No complex or abstract words.
        - No idioms, metaphors, or highly specialized terms.
        
        **2. Grammar & Sentence Structure (25 points)**
        - Uses simple present tense and basic past tense.
        - Uses short, simple sentences
        - Uses simple connectors (and, but, because).
        - Uses basic question forms (What, Where, When, Who).
        - No complex grammatical structures (e.g., passive voice, advanced conditionals, reported speech).
        - No future or perfect tenses.
        - **Focuses on concrete, immediate experiences.**
        ...
        **3. Content Preservation (25 points)**
        ...
        - **SCORING FOCUS: If the overall story flow is understandable, score generously**
        - **ACCEPTABLE: Minor details can be simplified or omitted as long as the main story is clear**
        - **KEY POINT: Focus on whether A1 learners can follow the basic story, not on preserving every detail**
        ...
        **4. Context & Continuity (25 points)**
        ...
        - **SCORING FOCUS: If the overall flow is understandable, score generously**
        - **ACCEPTABLE: Basic story connection is sufficient, complex context weaving not required**
        - **KEY POINT: Focus on whether A1 learners can follow the story flow, not on sophisticated context preservation**
    ...
    <scoring_system>
        **Scoring System:**
        - **90-100 points**: PASS - Excellent A1 level adaptation
        - **80-89 points**: PASS - Good A1 level adaptation  
    ...
    <output_format>
        ...
        **DETAILED FEEDBACK REQUIREMENTS:**
        - **Quote specific words or phrases** that are problematic (e.g., "The word 'sophisticated' in sentence 3 is above A1 level")
        - **Reference specific sentences** by number or content (e.g., "Sentence 'The magnificent castle stood proudly' uses vocabulary above A1 level")
        ...
        - Example: "Issue 1: Word 'sophisticated' in sentence 2 is above A1 level - replace with 'good'. Issue 2: Sentence 'The magnificent castle stood proudly' is too complex - change to 'The castle is big'. Issue 3: Use 'Tom' instead of 'he' in sentence 4 to avoid confusion. Issue 4: Context mismatch - context is about cooking recipes but leveled text discusses computer programs - completely unrelated topics."
    </output_format>"""

    human_prompt_intro = """
    <human_prompt_intro>
        You are an expert CEFR evaluator specializing in A1 level text assessment. Your job is to evaluate whether a leveled text meets A1 standards.
        
        Please evaluate this A1 level text adaptation:
        ...
        **A1 Level Version:**
        ...
        Then evaluate the A1 version against the original, considering:
        - Does it maintain A1 vocabulary and grammar standards?
        - Is the original meaning preserved?
        - Does it connect properly with the context?
        - Is it appropriate for basic beginners?
